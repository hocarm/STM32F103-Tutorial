...
/*	if (handle->uartBuf[0] == PMS_START_BYTE_1) { //0x42 first start character
			HAL_UART_Receive_IT(pms7003.uart, handle->uartBuf, 1);
			handle->state = PMS7003_RECEIVING;
		}
		if (handle->uartBuf[0] == PMS_START_BYTE_2 && handle->countDataToReceive == 31) {
			int check_code = PMS_START_BYTE_1 + PMS_START_BYTE_2;
			for (uint8_t i = 1; i <= 28; i++) {
				check_code = +handle->uartBuf[i];
			}
			if (check_code == ((handle->uartBuf[29] << 8) + handle->uartBuf[30])) {
				...
				handle->countDataToReceive = 1;
				handle->state = PMS7003_RECEIVED;
			}
			else handle->state = PMS7003_CRC_ERROR;
		}
		else handle->countDataToReceive = 1; */
}
static void PMS7003_Init(void){
	pms7003.uart = &huart2;
	pms7003.uartNumber = 2;
	pms7003.countDataToReceive = 1;
	pms7003.HEATER_gpioPin = HEATER_ON_PIN_Pin;
	pms7003.HEATER_gpioPort = HEATER_ON_PIN_GPIO_Port;
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOA, HEATER_ON_PIN_Pin|PMS_ON_PIN_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pins : HEATER_ON_PIN_Pin PMS_ON_PIN_Pin */
  GPIO_InitStruct.Pin = HEATER_ON_PIN_Pin|PMS_ON_PIN_Pin;
