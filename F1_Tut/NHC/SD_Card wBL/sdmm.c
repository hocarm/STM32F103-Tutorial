...
#define	CS_H()		GPIO_SetBits(GPIOA, GPIO_Pin_4) //PORTB |= 0x08	/* Set MMC CS "high" */
#define CS_L()		GPIO_ResetBits(GPIOA, GPIO_Pin_4) //PORTB &= 0xF7	/* Set MMC CS "low" */
...
{
	do {
		*buff++ = My_SPI_Exchange(0xFF);			/* Store a received byte */
	} while (--bc);
}
